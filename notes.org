...
and back-propagating action potentials [CITATIONS]. At at V$_{rest}, however, the neuronal
...
of PTs in experimental studies [CITATIONS], so we also used linear chirp stimulation in 
...
logarithmic, etc.) and white-noise [CITATIONS].  White-noise has the advantages of requiring
...
"We also evaluated the use of white-noise, rather than linear chirp, stimuli to
compute impedance amplitude and phase.  We found that there are sizeable errors in 
impedance phase at frequencies greater than 100 Hz, but since we only use this method
for a qualititative comparison of impedance phase across a large frequency bandwidth,
we consider these errors acceptable for our purposes."
...
what is seen in the realistic models.
...
(\fref{toymodel}C) are both roughly 7 Hz, much like what is seen in real PTs (CITATIONS). 
...
higher (around 70 Hz, \fref{toymodel}B}), but impedance phase is higher than in the passive
...
have added a panel to this figure showing the transfer impedance phase up to 500 Hz
...
100 Hz.  In this range, we see that impedance phase is highest for the control across 
this frequency range, blocking shunting current shifts impedance phase down across the
...
*****  Please see response to Reviewer #1, Major Concern #4.
