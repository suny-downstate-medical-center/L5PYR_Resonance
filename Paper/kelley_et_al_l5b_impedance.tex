...
\begin{equation}  Z_i{_n} = \frac{FFT(I_i{_n})}{FFT(V_m)} \label{eq:01}\end{equation}
...
extract the real valued resitance (R) and the imaginary valued reactance ($X$).  From R and $X$ can calculate the impedance amplitude:
...
transfer frequency specifies the frequency at which $|$Z$_c|$ is maximized.  Meanwhile, 
$\Phi$ defines the temporal relationship between I$_{in}$ and membrane potential. When 
$\Phi$ $>$ 0, the changes in membrane potential lead  I$_{in}$, which is referred to as leading phase.
When $\Phi$ = 0 I$_{in}$ and membrane potential are synchronous, and otherwise I$_{in}$ leads the membrane potential.
A leading is driven by the balance of capacitive and inductive reactances 
\citep{Das2017-nz}.
