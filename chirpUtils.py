...
def conditionAndTest(stim_seg, soma_seg, Sc0, St0, dSt, start, lag):
    # setup recordings
    v_stim = h.Vector()
    v_soma = h.Vector()
    t_vec = h.Vector()
    ...
    condStim.number = 1
    condNC = h.NetCon(condStim, condSyn)
    condNC.weight[0] = Sc0

    # test stimulus
    testSyn = h.ExpSyn(stim_seg)
    testSyn.tau = 2
    testStim = h.NetStim()
    testStim.start = start + lag
    testStim.number = 1
    testNC = h.NetCon(testStim, testSyn)
    ...
    return St0, v_soma, v_stim, t_vec 

def findSc(stim_seg, soma_seg, start, sc, dWeight):
    # setup recordings
    v_soma = h.Vector()
    t_vec = h.Vector()
    v_soma.record(soma_seg._ref_v)
...
def getTp(stim_seg, start, sc):
    ...
    # dV = np.diff(v_stim.as_numpy())
    # inds = np.argwhere(np.abs(dV) > 0.01)
    # tReturn = t_vec[inds[-1][0]]
    # Tp = tReturn - start 
