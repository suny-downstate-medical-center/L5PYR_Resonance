...
the response, which is not an issue for the chirp stimulus."
...
we consider white-noise acceptable for that purpose.  Therefore, when computing impedance 
for the simple models seen in \fref{toymodel}, we 
used a 5 s sinusoid at each frequency (0.5 - 1000 Hz in 0.5 Hz increments) rather than chirp or white-noise."
...
Please see response to Reviewer #1, Major Concern #4 for details.
...
***** Vaidya and Johnston demonstrate that realistic transfer resonance frequencies and 
...
are either linear or sigmoidal (\fref{compareHayDura}).  The shape of a sigmoidal distribution (where channel density increases along the apical trunk and 
saturates in the apical tufts) \cite{Harnett2015-sj}, as well as passive intracellular
resistivity \cite{Fletcher2019-sm}, are both dependent on dendritic length. Futhermore, experimental evidence shows morphological, electrophysiological, and
computational properties of PTs are closely related to anatomical location. While these morphologically-related factors no doubt 
influence the properties of dendritic impedance, their precise contributions to resonance and synchrony are beyond the scope of this study. 
Here we show two PT models with different apical trunk lengths and different distributions of HCN and TASK-like channels proucing  
similar dendritic impedance properties, showing how degenracy of the impedance profiles may emerge.  However, factors like morphology and 
intracellular resistivity may be strong contributors to the experimentally observed variance in impedance properties \cite{Ulrich2002-dd, Dembrow2015-zb}.
